    ...
    private void showSessions(ActionEvent event) {
        ...
        try {
            ...
            if (token == null || !sessionService.isValid(token)) {
                auditLogService.log(current.getEmail(), "SESSION_EXPIRED", "Session expired (Etudiant dashboard)");
                UserSession.clear();
                loadPage(event, "/Login.fxml");
                return;
            }
            ...
            a.showAndWait().ifPresent(btn -> {
                if (btn == disconnectAll) {
                    try {
                        ...
                        auditLogService.log(current.getEmail(), "SESSION_REVOKE_ALL", "User revoked all sessions (Etudiant dashboard)");
                    } catch (Exception ignored) {
                    }
                    UserSession.clear();
                    loadPage(event, "/Login.fxml");
